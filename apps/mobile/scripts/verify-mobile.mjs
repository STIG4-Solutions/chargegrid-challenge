...
check('nada mudou, nada e enviado', mesmo(corpoDaEdicao(carro, camposIniciais(carro)), {}))
check(
  'so a placa mudou, so a placa vai',
  mesmo(corpoDaEdicao(carro, { modelo: 'Nissan Leaf', placa: 'XYZ9K88', bateria: '40' }), {
    plate: 'XYZ9K88'
  })
)
check(
  'espaco em volta nao conta como mudanca',
...
check(
  'placa apagada vira null',
  mesmo(corpoDaEdicao(carro, { modelo: 'Nissan Leaf', placa: '', bateria: '40' }), { plate: null })
)
check(
  'bateria apagada vira null',
...
check(
  'bateria em virgula chega convertida',
...
check(
  'modelo vazio impede salvar',
  problemaNaEdicao(camposIniciais({ ...carro, model: '' })) !== null
)
check(
  'modelo so com espaco impede salvar',
  problemaNaEdicao({ modelo: '   ', placa: '', bateria: '' }) !== null
)
check(
  'modelo longo demais impede salvar',
...
check(
  'bateria com letra impede salvar',
  problemaNaEdicao({ modelo: 'Leaf', placa: '', bateria: 'quarenta' }) !== null
)
check(
  'edicao valida nao reclama',
...
check('sugestoes sem repetir e em ordem', mesmo(centrosEmUso(frota), ['Comercial', 'Logística']))
...
check(
  'frota toda classificada nao tem pendencia',
...
check('nome de uma letra nao passa', problemaNoCadastro({ ...CADASTRO, nome: 'M' }) !== null)
check('nome de duas letras passa', problemaNoCadastro({ ...CADASTRO, nome: 'Ma' }) === null)
check(
  'senha de 7 nao passa',
...
check('corpo nao carrega role nem site_id', !('role' in corpo) && !('site_id' in corpo))
...
check(
  'outro erro mostra o detalhe do servidor',
